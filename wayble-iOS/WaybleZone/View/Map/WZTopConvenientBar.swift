...
enum WZConvenient: String, CaseIterable, Identifiable, Hashable {
    case elevator = "엘리베이터"
    case ramp = "경사로"
    case toilet = "장애인 화장실"
...
struct WZTopConvenientBar: View {
    var onSelect: (WZConvenient) -> Void = { _ in }
    ...
    @State private var selected: WZConvenient? = nil
    private let rows = [GridItem(.flexible())]
    
    var body: some View {
        ScrollView(.horizontal, showsIndicators: false) {
            LazyHGrid(rows: rows, spacing: 10) {
                ForEach(WZConvenient.allCases) { kind in
                    ConvenientButton(
                        title: kind.rawValue,
                        icon: kind.icon,
